...
pub async fn create_subscription(configuration: &configuration::Configuration, body: Option<crate::models::CreateSubscriptionRequest>) -> Result<crate::models::Subscription, Error<CreateSubscriptionError>> {
...
pub async fn disable_subscription(configuration: &configuration::Configuration, subscription_id: &str) -> Result<crate::models::Subscription, Error<DisableSubscriptionError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/subscriptions/{subscriptionId}", local_var_configuration.base_path, subscriptionId=crate::apis::urlencode(subscription_id));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DisableSubscriptionError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn get_subscription(configuration: &configuration::Configuration, subscription_id: &str) -> Result<crate::models::Subscription, Error<GetSubscriptionError>> {
    let local_var_configuration = configuration;
...
pub async fn list_subscriptions(configuration: &configuration::Configuration, event_type: Option<&str>, page_size: Option<i32>, page_token: Option<&str>) -> Result<crate::models::SubscriptionList, Error<ListSubscriptionsError>> {
    let local_var_configuration = configuration;
